...
export class IssueComponent implements OnInit {
  ...
  addUrl() {
    const id = this.route.snapshot.paramMap.get('id');
    const urlInput = (<HTMLInputElement>document.getElementById('urlInput')).value;
    const urlTitleInput = (<HTMLInputElement>document.getElementById('urlTitleInput')).value;
  ...
  addTroubleshooting() {
    const id = this.route.snapshot.paramMap.get('id');
    this.urlsService
      .addTroubleshootingByIssueId(this.troubleshooting, id)
      .subscribe(() => {
        window.location.reload(true);
      });
  }
